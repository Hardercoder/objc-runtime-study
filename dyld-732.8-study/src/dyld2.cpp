...
void initializeMainExecutable()
{
	...
	if ( rootCount > 1 ) {
		for(size_t i=1; i < rootCount; ++i) {
			// 获取所有的ImageLoader，执行他们的runInitializers方法
...
fake_main()
{
	// 这里我们搜索__dyld_start发现一个神器
	// 我们可以设置环境变量DYLD_SKIP_MAIN，让dyld不返回我们工程的main函数地址，而是来调用函数签名一样的fake_main
	// 可以达到对整个程序启动偷梁换柱的作用
	return 0;
}
...
//
// ok,我们找到了__dyld_start调用的_main函数，这个函数会返回我们工程main函数的地址以供启动
// 这里表明是dyld的启动函数
uintptr_t
_main(const macho_header* mainExecutableMH, uintptr_t mainExecutableSlide, 
		...
		uintptr_t* startGlue)
{
	// debug环境下是否可以trace dyld
	if (dyld3::kdebug_trace_dyld_enabled(DBG_DYLD_TIMING_LAUNCH_EXECUTABLE)) {
		// 本源码中没找到launchTraceID的使用的地方，猜测应该是另一个专门的进程或线程id
	...
	// oh my god，发现一个新词CDHash，继续谷歌http://xelz.info/blog/2019/01/11/ios-code-signature/
	// 先计算被签名数据的哈希，然后再对哈希值进行签名，CDHash(Code Directory’s Hash)
	// 这个原理和https内容传输的手法属于一个模子
...
#if !TARGET_OS_SIMULATOR
	// 追踪dyld和执行文件的加载 内部调用kdebug_trace_dyld_image
	// Trace dyld's load
	...
	uintptr_t result = 0;
	// 因为ASLR，所以需要全局知道mainExecutableSlide，sMainExecutableMachHeader
	...
	// AMFI是个啥东西？？一脸问号。好的，现在有疑问的同学可以看下这个了https://www.jianshu.com/p/96837976c1f8
	//iOS 中的“坏警察”的职责是由Apple Mobile File Integrity.kext(AMFI)扮演的
...
reloadAllImages:
		...
		// load any inserted libraries 加载插入的二进制库，应该就是静态库吧
	...
	#else
		// run all initializers 进行执行文件的的初始化操作
		initializeMainExecutable(); 
	#endif
