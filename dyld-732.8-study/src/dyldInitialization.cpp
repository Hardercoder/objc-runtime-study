...
// 磁盘上dyld的数据段的所有指针链接在一起，需要修复他们为真的指向才可运行
static void rebaseDyld(const dyld3::MachOLoaded* dyldMH)
{
    ...
    // 所有的基于images的fixup chain都有一个0基址，所以可以用slide = ma，此时ma就是load address
				...
				const dyld3::MachOLoaded* dyldsMachHeader, uintptr_t* startGlue)
{
    ...
    // 触发kdebug tracepoint指示dyld的bootstrap已经开始了
    dyld3::kdebug_trace_dyld_marker(DBG_DYLD_TIMING_BOOTSTRAP_START, 0, 0, 0, 0);

	// if kernel had to slide dyld, we need to fix up load sensitive locations
	// we have to do this before using any global variables
    // 如果内核必须slide dyld，我们需要修复 敏感的加载位置
    // 必须在使用任何的全局变量前做这一步处理
    // 传说中的rebase操作
    ...
    // 难道这是传说中的ASLR，Address space layout randomization
	__guard_setup(apple);
...
#endif
    // 做完bootstrap的引导工作后，调用dyld的主执行函数_main
